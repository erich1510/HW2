...
class Graph:
    ...
    # Builds a generic graph, with no edge information specified...
    # The variable lastNode keeps track of the next node with
    # qData *undefined*, available to be defined later
    ...
    # -------------------------
    # First, some operations for adding qData and edges to the graph

    # Add qData to the next available node. It returns the index of the
    # node, and it raises an exception if no more nodes are available
    # to have qData attached to them.
    def addNodeData(self, nodeData):
        """Takes a new node qData item, and adds it to the next available
        node.  If no node is available, then it raises an exception, otherwise it
        returns the index of the node to which this qData was added"""
    ...
    def getData(self, node):
        """Takes in a node index, and returns the qData associated with
        the node, if any."""
    ...
    def findNode(self, data):
        """Takes in a qData item, and returns the node index that contains
        the qData item, if it exists.  Otherwise it raises an exception"""
    ...
    def areNeighbors(self, node1, node2):
        """Takes in two node indices, and returns True if they are neighbors
...
# ======================================================================
class DirectedMatrixGraph(Graph):
    """A graph contains vertices and edges: This implementation uses
...
# End class DirectedMatrixGraph


# ======================================================================
class DirectedListGraph(Graph):
    """A graph contains vertices and edges: This implementation uses
    an adjacency list to represent edges."""
    ...
    def areNeighbors(self, node1, node2):
        """Takes in two node indices, and returns True if they are neighbors
        and False if they are not. First node is the source, second is the to node"""
        if node1 < self._numVerts and node2 < self._numVerts:
            return (node2 in self._adjList[node1])
        elif node1 >= self._numVerts:
            raise NodeIndexOutOfRangeException(0, self._numVerts, node1)
...
class WeightedListGraph(ListGraph):
    ...
    # be dealt with, and in areNeighbors, where the weight must be dealt with.
    ...
    # the weight between them, or False if they aren't neighbors.
    ...
    def getWeight(self, node1, node2):
        ...
        or None if they are not neighbors."""
        if node1 < self._numVerts and node2 < self._numVerts:
            for (n, w) in self._adjList[node1]:
                if n == node2:
                    return w
...
class WeightedMatrixGraph(MatrixGraph):
    ...
    # areNeighbors works without modifications, but
...
class GraphFullException(Exception):
    ...
    def __str__(self):
        s = "No more node qData may be added: all nodes are in use"
        return s


class NoSuchNodeException(Exception):
    """A special exception for catching when node qData is input that
    doesn't match any node qData in the graph"""

    def __init__(self, data):
        self.data = data

    def __str__(self):
        s = "Node qData " + str(self.data) + \
            " not assigned to any node in the graph"
        return s
