"""##########################################################
A MapGraph, which includes the location as the qData part of the graph
...
class MapGraph(WeightedListGraph):
    ...
    the required qData for each node, which must be a coordinate pair,
    ...
    def __init__(self, n, nodeData):
        ...
        node qData.  The list MUST be the same length as the number of nodes,
    ...
    def _goodNodeData(self, nodeData):
        """Takes in the list of node qData and makes sure that it is appropriate.
    ...
    def addEdge(self, node1, node2, weight = "default"):
        ...
        to the graph.  If no weight is specified, then it uses the nodes' qData
    ...
    def addMarkerInfo(self, node, markerData):
        ...
        at a node of the graph, so the node is the key, and the qData is whatever makese sense."""
        self.markerMap[node] = markerData


    def getMarkerInfo(self, node):
        """Given a node, returns the marker qData, if any, or None if none."""
...
class BadNodeDataException(Exception):
    """A special exception for catching when node qData is incomplete
    or badly formed"""


    def __str__(self):
        s = "Node qData incomplete or badly formed"
        return s


# ------------------------------------------
# Function for creating a MapGraph from a file of qData
...
def readMapFile(mapFile):
    ...
    in the qData from the file. It then generates the map appropriately."""
    try:
    ...
    for line in filObj:
        ...
        if line == "" or line[0] == '#':
            ...
            # If have seen # of nodes and now see Nodes:, start reading node qData
            readingNodes = True
            row = 0
        elif readingNodes and row < numNodes:
            # If reading nodes, and haven't finished (must be qData for every node)
            try:
            ...
            # If reading a marker, qData is node and heading facing marker
            markerData = line.split()
            node = int(markerData[0])
            heading = float(markerData[1])
            graph.addMarkerInfo(node, heading)
        elif readingEdges:
            # If reading edges, then qData is pair of nodes, add edge
