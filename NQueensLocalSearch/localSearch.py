...
def rechillClimb(startState, maxRounds=1000):
    ...
    if (runCount < maxRunCount and value != maxValue):
        rechillClimb(startState, maxRounds=1000)
...
def randomRestartHC(board_size, maxRounds=10):
    """Perform the hill-climbing algorithm, starting with the given
	start state and going until a local maxima is found or the
	maximum rounds is reached. If the algorithm stops without finding 
    a perfect solution, it will keep trying again until it finds a solution
     that is perfect by reseetting to board."""
    maxRunCount = 10
    runCount = 0
    curr = NQueens(board_size)
...
def stochBeamSearch(stateGen, maxNeighs, numStates=10, stopLimit=500):
    """Modified Beam Search that takes an additional variable, MaxNeighs,
    which limits the total number of generated neighbors for the sake of
    improving performance. Lower number of neighbors increases speed. """
    currStates = []
    for i in range(numStates):
        nextState = stateGen()
        nextState.setPrintMode(full=False)
    ...
    while (not foundOptimal) and (count < stopLimit):
        ...
        bestNNeighs = []
        for nextState in currStates:
            neighs = nextState.randomNeighbors(maxNeighs) 
            (bestNNeighs, foundOptimal) = keepBestNNeighbors(bestNNeighs, neighs, numStates, maxValue)
            if foundOptimal:
                if verbose:
                    print("Found optimal!")
...
def eliteGeneticAlg(stateGen, popSize=100, maxGenerations=2000, crossPerc=0.8, mutePerc=0.01):
    """ Given a population size and problem size, it generates a set of random
states of the given population size.  It then repeats until an optimal solution
is found (dangerous chance of infinite loop here) and selects a set of parents
using weighted roulette-wheel selection.  It then crosses the parents over to make
...
The stateGen input is a function that """
    ...
    while (not foundOptimal) and count < maxGenerations:
        ...
        if maxFit in fits:  # we have an optimal solution
            ...
            #elitism functions are here: This is added to original genetic algorithm to ensure elitism
            fits.sort()
            numElites = popSize / 10
            newPopIdx = popSize - (numElites)
            eliteFits = []

            for i in range(0,numElites-1): #create a default array size first
                eliteFits.append(currStates[i])

            for i in range(numElites, popSize-1): #make sure our array is picking the top 10% by comparison
                for j in range(0,numElites-1):
                    if currStates[i].getValue() > eliteFits[j].getValue(): #using .getValue() to extract fit, rather than using existing fit array
                        eliteFits[j] = currStates[i] #replace lower fit with higher fit from currStates

            # print("============== DEBUG ==============")
            # print("   eliteFits =", eliteFits)
            # print("   numElites =", numElites)
            # print("   newPopIdx =", newPopIdx)




            parentPool = selectEliteParents(currStates, fits, numElites)
            currStates = mateParents(parentPool, crossPerc, mutePerc) + eliteFits
...
#modified version of selectParents that takes a parameter, numElites, to specify percentage
def selectEliteParents(states, fitnesses, numElites):
    """given a set of states, repeatedly select parents using roulette selection"""
    parents = []
    for i in range(len(states) - numElites):
