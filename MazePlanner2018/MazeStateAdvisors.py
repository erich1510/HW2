...
class MacState(SearchState):
    ...
    def __init__(self, x, y, id, path=None, cost=None):
    ...
    def __eq__(self, state):
        """Check if the input is the same type, and if it has the same x and y
        Overloads the == operator."""
        if type(state) is type(self):
            return (state.id == self.id) & (state.x == self.x) & (state.y == self.y)
        else:
            return False
    ...
    def __str__(self):
        ...
        path and cost"""
        strng = "[" + str(self.x) + ", " + str(self.y) + ", " +  str(self.id) + "]"
...
class GraphAdvisor(AbstractTaskAdvisor):
    ...
    def __init__(self, graph, startX, startY, endX, endY, id):
        ...
        self.endY = endY
        self.id = id
        self.startState = self._setupInitialState(startX, startY)
    ...
    def generateNeighbors(self, state):
        ...
        neighs = graph.getNeighbors(x, y)
        return neighs


    def _buildNeighbor(self, currState, neighX, neighY, neighID):
        ...
        newPath.append(neighX, neighY)
        return MacState(newPath, neighX, neighY, neighID)




class UCSGraphAdvisor(GraphTaskAdvisor):
    ...
    def _setupInitialState(self, startRow, startCol):
        ...
        return MazeState(startRow, startCol, [], 0)
    ...
    def _buildNeighbor(self, currState, neighX, neighY, neighID):
        ...
        newPath = currState.getPath()[:]
        newPath.append(neighX, neighY, neighID)
        oldCost = currState.getCost()


        lastNode = newPath[len(newPath)]
        newCost = self.graph.heuristicDist(lastNode[2], neighID)
        return MacState(neighX, neighY, neighID, newPath, oldCost + newCost)

    class AStarGraphAdvisor(GraphTaskAdvisor):
    """This class is a subclass of the MazeTaskAdvisor. It implements the cost calculations
    ...
    def _setupInitialState(self, startX, startY):
        """This creates and returns a proper start state for this particular
        ...
        """
        g = 0
        h = self._calcDistToGoal(startX, startY)
        return AStarMazeState(startX, startY, id, [], g, h)
        ...
        class. In this case cost is the distance travelled so far, and that
        starts at whatever the starting position has in it."""
        return MazeState(startX, startY, [], 0)
    ...
    def _buildNeighbor(self, currState, direction, neighRow, neighCol):
        ...
        return AStarMazeState(neighRow, neighCol, newPath, newG, newH)
    ...
    def _calcDistToGoal(self, row, col):
        ...
        xDist = abs(row - self.endX)
        yDist = abs(col - self.endY)
        return xDist + yDist
...
#         return AStarMazeState(startRow, startCol, [], g, h)
#
#     def _buildNeighbor(self, currState, direction, neighRow, neighCol):
#         """Given the current state and the location of the neighbor, this builds
#         a new state, computing the cost as appropriate for the class.
